...
def bot_can_delete(func):
    @wraps(func)
    def delete_rights(update: Update, context: CallbackContext):
        if can_delete(update.effective_chat, context.bot.id):
            return func(update=update, context=context)
        else:
...
def can_pin(func):
    @wraps(func)
    def pin_rights(update: Update, context: CallbackContext):
...
def can_promote(func):
    @wraps(func)
    def promote_rights(update: Update, context: CallbackContext):
...
def user_not_admin(func):
    @wraps(func)
    def is_not_admin(update: Update, context: CallbackContext):
        user = update.effective_user  # type: Optional[User]
